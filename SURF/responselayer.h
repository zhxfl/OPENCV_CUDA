...
public:
	...
	float* get_device_response()
	{
		if(!d_response)
		{
			d_response = (float*) cudaMalloc((void**)&d_response, width * height * sizeof(float));
			cudaMemcpy(d_response, responses,  width * height * sizeof(float), cudaMemcpyHostToDevice);
		}
		return d_response;
	}
	float* get_device_laplacian()
	{
		if(!d_laplacian)
		{
			d_laplacian = (uchar*) cudaMalloc((void**)&d_laplacian, width * height * sizeof(uchar));
